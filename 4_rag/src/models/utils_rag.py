...
class Seq2SeqDataset(Dataset):
    def __init__(
        ...
        super().__init__()
        if type_path == 'val' or type_path == 'test':
            type_path = 'validation'
        path = f'/opt/ml/input/data/data/moco_dataset/{type_path}'
        self.dataset = load_from_disk(path)
        self.src_lens = len(self.dataset)
    ...
    def __getitem__(self, index) -> Dict[str, torch.Tensor]:
        source_line = self.dataset[index]['question']
        tgt_line = self.dataset[index]['answers']
